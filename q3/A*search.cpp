...
int Asearch(vector<vector<char>>& grid, vector<vector<int>>& visited, int start_row, int start_column, int goal_row, int goal_column){
    int no_of_rows = grid.size();
    int no_of_columns = grid[0].size();

    // Define a min heap (priority queue) to store states with priority based on f-value which is eqaul to (g+h).
    ...
    // Calculate the heuristic value using the start position
    int heuristic = manhattanDistance(start_row, start_column, goal_row, goal_column);

    // Push the initial state onto the min heap with priority based on f value and cost
    min_heap.push({heuristic+0, 0, start_row, start_column});
        ...
        for (int dir = 0; dir < 4; ++dir) {
            ...
            if (newRow >= 0 && newRow < no_of_rows && newCol >= 0 && newCol < no_of_columns) {
                ...
                if (grid[newRow][newCol] != 'W' && !visited[newRow][newCol]) {
                    ...
                    int f = g+ heuristic;
                    // Adjust the heuristic value based on terrain cost from the start position
...
int main(){
    #ifndef ONLINE_JUDGE
   freopen("input_A*search.txt", "r", stdin);
   freopen("output_A*.txt", "w", stdout);
    ...
    // Call the A* search function and print the result
    cout<<"The minimum cost form S(start) to F(Finish). ";
    cout << Asearch(grid, visited, start_row, start_column, goal_row, goal_column) << endl;
